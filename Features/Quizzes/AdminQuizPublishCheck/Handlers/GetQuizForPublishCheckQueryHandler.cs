...
public sealed class GetQuizForPublishCheckQueryHandler(IGenericRepository<Quiz> quizRepository)
                                                      : IRequestHandler<GetQuizForPublishCheckQuery, Result<PublishCheckData>> 
{
    public async Task<Result<PublishCheckData>> Handle(
        GetQuizForPublishCheckQuery request,
        CancellationToken cancellationToken)
    {
        var quiz = await quizRepository
            .Get(x => x.Id == request.QuizId && !x.IsDeleted)
            .Select(x => new PublishCheckData(
