...
public sealed class GetDiplomasQueryHandler(IGenericRepository<Diploma> genericRepository,ICurrentUser currentUser,ISender sender)
    ...
    public async Task<Result<PaginatedResult<DiplomaListItemResponse>>> Handle(GetDiplomasQuery request,CancellationToken cancellationToken)
    {
        ...
        var items = await query
            ...
            .Select(diploma => new DiplomaListItemResponse(
                ...
                diploma.Title,
                diploma.Description,
                diploma.Quizzes.Count(quiz =>!quiz.IsDeleted &&quiz.Status == QuizStatus.Published &&quiz.Attempts.Any(attempt =>
                        attempt.StudentId == studentId.Value &&(attempt.Status == AttemptStatus.Submitted ||attempt.Status == AttemptStatus.TimedOut))),
